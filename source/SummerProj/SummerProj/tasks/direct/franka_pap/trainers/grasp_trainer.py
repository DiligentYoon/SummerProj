...
class GraspTrainer(Trainer):
    ...
    def single_agent_train(self) -> None:
        ...
        for timestep in tqdm.tqdm(
            ...
            with torch.no_grad():
                # compute actions
                actions = self.apply_action_curriculum(states=states, timestep=timestep)
    ...
    # =======================================================================================
    # ========================= Train Loop Auxiliary Function ===============================
    # =======================================================================================
    
    def apply_action_curriculum(self, states: torch.Tensor, timestep: int) -> torch.Tensor:
        retract_envs_mask = self.env._unwrapped.is_place
        actions = self.agents.act(states, timestep=timestep, timesteps=self.timesteps)[0]

        if torch.any(retract_envs_mask):
            random_prob = torch.rand(self.env.num_envs, device=self.env.device)
            override_mask = retract_envs_mask & (random_prob < self.env._unwrapped.select_demo_prob)
            actions[:, 21] = torch.where(override_mask, torch.abs(actions[:, 21]), actions[:, 21]) 
