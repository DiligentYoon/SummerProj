...
class FrankaGraspEnv(FrankaBaseEnv):
    ...
    def _get_rewards(self):
        ...
        # Place Error Bonus
        phi_s_prime_place_loc = -torch.log(self.cfg.alpha * self.place_error[:, 0] + 1)
        phi_s_prime_place_rot = -torch.log(self.cfg.alpha * self.place_error[:, 1] + 1)
        phi_s_place_loc = -torch.log(self.cfg.alpha * self.prev_place_error[:, 0] + 1)
        phi_s_place_rot = -torch.log(self.cfg.alpha * self.prev_place_error[:, 1] + 1)
